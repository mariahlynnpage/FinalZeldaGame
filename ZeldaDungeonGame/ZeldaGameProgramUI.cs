...
namespace ZeldaDungeonGame
{
    class ZeldaGameProgramUI
    {
        ...
        static int zorasHealth = 7;
        static int zorasAttack = 1;
        ...
        private static void AttackLoop()
        {
            ...
            if (enemyHealth <= 0)
            {
                Console.WriteLine("You proceed to Zora's Domain tower");
                Console.ReadLine();
                ZoraAttackLoop();
            }
            void ZoraAttackLoop()
            {
                ...
                {
                    Console.WriteLine("You currently have " + linksHearts + "hearts left");
                    Console.WriteLine("The enemy has" + zorasHealth + "hearts left");
                    Console.WriteLine("Will you use your sword or defend with your shield?");
                    linksChoice = Console.ReadLine(); Console.WriteLine();
                    if (linksChoice == "sword")
                    {
                        Random rand = new Random();
                        int Zora = (rand.Next(1, 5) * linksAttack);
                        zorasHealth = zorasHealth - Zora;
                        Console.WriteLine("The Zora uses its water attack on you!");
                        Random random = new Random();
                        int Zora1 = (random.Next(1, 5) * zorasAttack);
                        linksHearts = linksHearts - Zora;
                        Console.WriteLine("You hit the Zora, it took {0} damage!", Zora);
                        Console.WriteLine("Link took {0} damage from the Zora!", Zora);
                    }
                    else if (linksChoice == "shield")
                    {
                        Console.WriteLine("The siren attacks you!");
                        Random rand = new Random();
                        int Zora2 = (rand.Next(1, 5) * zorasAttack);
                        linksHearts = linksHearts - Zora2 / 2;
                        Console.WriteLine("Link took {0} hearts from the Zora!", Zora2 / 2);
                    }
                    else
                    {
                        Console.WriteLine("You hear a fairy nearby...");
                    }
                } while (linksHearts > 0 && zorasHealth > 0);
                if (zorasHealth <= 0)
                {
                    Console.WriteLine("You defeated the enemy!");
                    Console.ReadLine();
